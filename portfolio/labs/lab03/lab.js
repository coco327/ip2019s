...
for(var i = 0, f; f = files[i]; i++){
    ...
    reader.onload = functiion(evt){
        ...
        vae span = document.createElement('sapn');
        span.setAttribute("class", "svgtext");
        sapn.textContent = evt.target.result;
...
function handleFileSelect(evt) {
    var files = evt.taeget.files;

    readFiles(files);
}

function handleFilesSelect_drag(evt) {
    evt.stopPropagation();
    evt.preventDefault();
